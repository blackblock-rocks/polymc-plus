...
public class ItemBlockPoly implements BlockPoly {
    ...
    private void processModdedStates(List<BlockState> modded_states, PolyRegistry registry) {
        ...
        for (BlockState modded_state : modded_states) {
            ...
            // Get all the multipart variants somehow
            ...
            // Add the main variant if there is one
            if (modded_variants != null && modded_variants.length > 0) {
                JBlockStateVariant main_variant = modded_variants[0];

                Integer x = main_variant.x();
                Integer y = main_variant.y();
                String model = main_variant.model();
            ...
            // Add all the multipart variants
            for (JBlockStateVariant variant : multipart_variants) {
